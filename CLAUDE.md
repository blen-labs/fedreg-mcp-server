...
- A green `pnpm test` on a machine with NO sandbox runner does not prove the sandbox works: sandbox-execution tests are gated on `await runner.available()` and report a skip when their runner is unavailable. CI installs isolated-vm AND Deno, so both runners' gated tests execute there.
...
- MCPB packaging: `pnpm build && pnpm mcpb:prepare && pnpm mcpb:pack` produces `fedreg-mcp-server.mcpb` (manifest is built by `buildManifest()` in `scripts/mcpb-prepare.mjs` and schema-validated during prepare). The prepare step's `pnpm install` MUST keep `--ignore-workspace` — without it pnpm walks up to the repo's `pnpm-workspace.yaml` and `--prod`-prunes the ROOT `node_modules` instead of installing the bundle's. `--config.node-linker=hoisted` is also required (symlinked layouts don't survive zipping). isolated-vm ships unbuilt (`--ignore-scripts`), so `execute` inside the bundle needs Deno on the host or reports `SandboxUnavailable`.
...
- Version strings live in three places (package.json, `src/server/mcpServer.ts`, `src/sdk/bindings.ts` `version()`). Releases are continuous: EVERY push to main is released by `.github/workflows/release.yml` — `scripts/release.mjs` bumps all three (the two source lines carry `x-release-version` markers — don't remove them), prepends the CHANGELOG section, tags, and publishes via OIDC. Commit types set the bump (breaking → major, `feat` → minor, anything else → patch); never hand-edit versions or tags. See CONTRIBUTING "Releasing".
